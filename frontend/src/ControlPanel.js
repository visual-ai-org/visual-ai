...
export default function ControlPanel({
  ...
  const [trainData, setTrainDataInput] = useState(
    "[[0, 0], [0, 1], [1, 0], [1, 1]]"
  );
  ...
  return (
    <Drawer
      ...
      anchor="right"
    >
      <Button onClick={handleClickOpen}>Set Training Data</Button>
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle>Set Training Data</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Please enter the training data and labels in JSON format, as well as
      ...
      <DialogContent>
        ...
        <List>
          {Array.from({ length: layers }, (_, index) => (
            <Layer index={index} lastIndex={layers - 1}>
              {" "}
            </Layer>
          ))}
        </List>
